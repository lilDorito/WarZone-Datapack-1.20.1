...
execute as @s[scores={temp_count=..191}] run tellraw @s {"text":"[✔] M-60 + ammo refilled","color":"green"}

execute as @s[scores={temp_count=192..}] run tellraw @s {"text":"[✔] M-60 equipped (ammo already full)","color":"yellow"}
