...
give @s superbwarfare:m_2_hb 1

execute store result score @s temp_count run clear @s superbwarfare:heavy_ammo 0

execute as @s[scores={temp_count=..63}] run clear @s superbwarfare:heavy_ammo
execute as @s[scores={temp_count=..63}] run give @s superbwarfare:heavy_ammo 64
execute as @s[scores={temp_count=..63}] run tellraw @s {"text":"[✔] Browning M2HB + ammo refilled","color":"green"}

execute as @s[scores={temp_count=64..}] run tellraw @s {"text":"[✔] Browning M2HB equipped (ammo already full)","color":"yellow"}
