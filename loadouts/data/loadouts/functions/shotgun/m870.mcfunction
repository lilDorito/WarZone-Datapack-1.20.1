...
execute as @s[scores={temp_count=..63}] run tellraw @s {"text":"[✔] M-870 MCS + ammo refilled","color":"green"}

execute as @s[scores={temp_count=64..}] run tellraw @s {"text":"[✔] M-870 MCS equipped (ammo already full)","color":"yellow"}
