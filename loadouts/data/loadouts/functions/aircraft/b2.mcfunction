...
give @s superbwarfare:container{BlockEntityTag:{id:"superbwarfare:container",EntityType:"ashvehicle:b-2",Count:1b}} 1
give @s superbwarfare:medium_aerial_bomb 8
give @s ashvehicle:gbu-57-item 2

tellraw @s {"text":"[✔] B-2 Spirit loadout received!","color":"green"}

execute unless score #game match_mode matches 4 if score #map map matches 13 run scoreboard players set @s aircraft_cd 54000
execute unless score #game match_mode matches 4 unless score #map map matches 13 run scoreboard players set @s aircraft_cd 18000