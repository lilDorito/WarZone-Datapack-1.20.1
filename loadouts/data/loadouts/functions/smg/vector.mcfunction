...
execute as @s[scores={temp_count=..191}] run tellraw @s {"text":"[✔] Vector + ammo refilled","color":"green"}

execute as @s[scores={temp_count=192..}] run tellraw @s {"text":"[✔] Vector equipped (ammo already full)","color":"yellow"}
