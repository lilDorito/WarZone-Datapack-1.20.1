...
execute as @s[scores={temp_count=..63}] run tellraw @s {"text":"[✔] SKS + ammo refilled","color":"green"}

execute as @s[scores={temp_count=64..}] run tellraw @s {"text":"[✔] SKS equipped (ammo already full)","color":"yellow"}
