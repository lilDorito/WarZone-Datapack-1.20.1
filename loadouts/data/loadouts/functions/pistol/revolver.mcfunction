...
give @s superbwarfare:trachelium 1
...
execute as @s[scores={temp_count=..63}] run tellraw @s {"text":"[✔] Trachelium Revolver + ammo refilled","color":"green"}

execute as @s[scores={temp_count=64..}] run tellraw @s {"text":"[✔] Trachelium Revolver equipped (ammo already full)","color":"yellow"}
