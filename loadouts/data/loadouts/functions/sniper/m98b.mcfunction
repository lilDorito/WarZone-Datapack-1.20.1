...
execute as @s[scores={temp_count=..31}] run tellraw @s {"text":"[✔] Barret M98B + ammo refilled","color":"green"}

execute as @s[scores={temp_count=32..}] run tellraw @s {"text":"[✔] Barret M98B equipped (ammo already full)","color":"yellow"}
