...
execute as @s[scores={temp_count=..31}] run tellraw @s {"text":"[✔] SVD + ammo refilled","color":"green"}

execute as @s[scores={temp_count=31..}] run tellraw @s {"text":"[✔] SVD equipped (ammo already full)","color":"yellow"}
