...
give @s superbwarfare:sentinel 1
...
execute as @s[scores={temp_count=..31}] run tellraw @s {"text":"[✔] Sentinel + ammo refilled","color":"green"}

execute as @s[scores={temp_count=31..}] run tellraw @s {"text":"[✔] Sentinel equipped (ammo already full)","color":"yellow"}
