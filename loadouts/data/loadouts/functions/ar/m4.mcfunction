...
execute as @s[scores={temp_count=..191}] run tellraw @s {"text":"[✔] M4 + ammo refilled","color":"green"}

execute as @s[scores={temp_count=192..}] run tellraw @s {"text":"[✔] M4 equipped (ammo already full)","color":"yellow"}
